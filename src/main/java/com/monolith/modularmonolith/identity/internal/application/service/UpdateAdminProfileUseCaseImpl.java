...
public class UpdateAdminProfileUseCaseImpl implements UpdateAdminProfileUseCase {
    ...
    public UserProfileResponse execute(String email, UpdateAdminProfileRequest request) {
        ...
        if (profile == null) {
            throw new ResourceNotFoundException("Profil administrateur", "email=" + email);
        }
        ...
        if (request.permissions() != null) profile.setPermissions(request.permissions());
        ...
        log.info("Profil admin mis à jour: {}", email);
